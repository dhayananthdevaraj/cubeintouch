...
//     // Pattern 3: "1st Week", "2nd Week", etc.
...
//     // Pattern 4: Module numbers like "Module 1", "Module 2"
...
//   const getDynamicWeekList = () => {
//     const weeksMap = new Map();
    
//     moduleTree.forEach(mod => {
//       const weekInfo = extractWeekIdentifier(mod.name);
//       if (weekInfo) {
//         weeksMap.set(weekInfo.identifier, weekInfo);
//       }
//     });
    
//     // Sort by order number
//     return Array.from(weeksMap.values()).sort((a, b) => a.order - b.order);
//   };

//   const selectWeekTests = (weekIdentifier) => {
//     const weekTestIds = new Set();
//     moduleTree.forEach(mod => {
//       const weekInfo = extractWeekIdentifier(mod.name);
//       if (weekInfo && weekInfo.identifier === weekIdentifier) {
//         mod.directTests.forEach(t => weekTestIds.add(t.id));
//         mod.subModules.forEach(sub => {
//           sub.tests.forEach(t => weekTestIds.add(t.id));
//         });
...
//     });
//     setSelectedTests(new Set([...selectedTests, ...weekTestIds]));
//     showAlert(`✅ Selected tests from ${weekIdentifier}`, "success");
//   };

//   const selectWeekPracticeTests = (weekIdentifier) => {
//     const practiceTestIds = new Set();
//     moduleTree.forEach(mod => {
//       const weekInfo = extractWeekIdentifier(mod.name);
//       if (weekInfo && weekInfo.identifier === weekIdentifier) {
...
//     });
//     setSelectedTests(new Set([...selectedTests, ...practiceTestIds]));
//     showAlert(`✅ Selected Practice tests from ${weekIdentifier}`, "success");
//   };

//   const selectWeekKCQTests = (weekIdentifier) => {
//     const kcqTestIds = new Set();
//     moduleTree.forEach(mod => {
//       const weekInfo = extractWeekIdentifier(mod.name);
//       if (weekInfo && weekInfo.identifier === weekIdentifier) {
...
//     });
//     setSelectedTests(new Set([...selectedTests, ...kcqTestIds]));
//     showAlert(`✅ Selected KCQ tests from ${weekIdentifier}`, "success");
//   };
...
//       const qbToTestMap = {};
//       const testQMap = {};
//       const qbMetadata = {}; // Store QB metadata including test types
...
//                 // Store metadata about rule-based tests
...
//                   {/* Dynamic Week Selection Dropdown */}
//                   {getDynamicWeekList().length > 0 && (
//                     <div className="qb-week-dropdown-container" style={{ position: 'relative', display: 'inline-block' }}>
//                       <button
//                         onClick={() => setShowWeekDropdown(!showWeekDropdown)}
//                         className="qb-button qb-button-small qb-button-info"
...
//                             background: 'white'
//                           }}>
//                             Select by Week/Module
//                           </div>
                          
//                           {getDynamicWeekList().map((weekInfo) => (
//                             <div key={weekInfo.identifier} style={{
//                               padding: '8px 12px',
//                               borderBottom: '1px solid #f5f5f5'
//                             }}>
//                               <div style={{
...
//                                 <button
//                                   onClick={() => {
//                                     selectWeekTests(weekInfo.identifier);
//                                     setShowWeekDropdown(false);
//                                   }}
//                                   className="qb-button qb-button-small"
//                                   style={{
...
//                                 <button
//                                   onClick={() => {
//                                     selectWeekPracticeTests(weekInfo.identifier);
//                                     setShowWeekDropdown(false);
//                                   }}
//                                   className="qb-button qb-button-small"
//                                   style={{
...
//                                 <button
//                                   onClick={() => {
//                                     selectWeekKCQTests(weekInfo.identifier);
//                                     setShowWeekDropdown(false);
//                                   }}
//                                   className="qb-button qb-button-small"
//                                   style={{
...
//                           ? 'linear-gradient(to right, #fff3e0 0%, #ffffff 100%)' 
//                           : undefined
//                       }}
//                     >
//                       <td className="qb-td">
...
export default function CourseQBFinder() {
  ...
  const extractWeekIdentifier = (moduleName) => {
    ...
    // PRIORITY 4: "1st Week", "2nd Week" pattern
    const ordinalMatch = lowerName.match(/(\d+)(?:st|nd|rd|th)\s*week/i);
    if (ordinalMatch) {
      return {
        identifier: `Week ${ordinalMatch[1]}`,
    ...
    // PRIORITY 5: "Module 1", "Module 2" pattern
    const moduleMatch = lowerName.match(/module\s*(\d+)/i);
    if (moduleMatch) {
      return {
        identifier: `Module ${moduleMatch[1]}`,
  ...
  const getAllModulesWithMetadata = () => {
    ...
    moduleTree.forEach((mod, index) => {
      ...
      const metadata = {
        ...
        display: mod.name,  // ALWAYS use full module name for display
        order: weekInfo ? weekInfo.order : 9000 + index,  // Non-pattern modules go last
    ...
    // Sort: pattern-based modules first (by order), then custom modules by index
  ...
  // Select tests by module (works for both patterned and non-patterned modules)
  ...
  // Select tests by global filter (across all modules)
  ...
  // Select module range (e.g., Module 1-5)
  ...
  const applyFilters = (questions) => {
    return questions.filter(q => {
      ...
      // Used status filter (only applies when showing all questions)
            ...
            <div className="qb-tree-section">
              <div className="qb-tree-header">
                ...
                <div className="qb-toggle-buttons">
                  ...
                  {/* Intelligent Dynamic Selection Menu */}
                  {moduleTree.length > 0 && (
                    <div className="qb-week-dropdown-container" style={{ position: 'relative', display: 'inline-block' }}>
                      <button
                      ...
                      {showWeekDropdown && (() => {
                        ...
                        return (
                          <div className="qb-week-dropdown" style={{
                            ...
                            {allModules.length > 0 ? (
                              <>
                                {/* Range Selection Section - Only show if patterned modules exist */}
                                ...
                                {/* Individual Module Selection - Shows ALL modules */}
                                <div style={{
                                  padding: '12px',
                                  background: '#f8f9fa',
                                ...
                                {/* Scrollable container for individual modules */}
                                <div style={{
                                  maxHeight: '400px',
                                  overflowY: 'auto',
                                  overflowX: 'hidden'
