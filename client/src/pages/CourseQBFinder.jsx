...
//       if (result.success || result.clonedQuestions) {
//         const clonedQIds = result.clonedQuestions || [];
//         setClonedQuestions(clonedQIds);
//         showAlert(`✅ Successfully cloned ${selectedQIds.length} question(s)!`, "success");
        
//         // IMPORTANT CHANGE: After cloning, fetch ALL questions from the QB
//         setShowAllQuestions(true);
//         showOverlay("📚 Fetching all questions from QB...");
        
//         try {
//           // Fetch all questions from QB
//           const allQuestions = await fetchQBQuestions(selectedQB.qb_id);
//           setAllQuestions(allQuestions);
//           setQbQuestions(allQuestions); // Show all questions
//           setSelectedQuestions(new Set(clonedQIds)); // Auto-select cloned questions
//           hideOverlay();
//           showAlert("✅ Now showing ALL questions from the QB. Cloned questions are highlighted.", "success");
//         } catch (err) {
//           hideOverlay();
//           showAlert("⚠️ Cloned but failed to fetch all questions: " + err.message, "warning");
//         }
//       } else {
//         showAlert("⚠️ Questions cloned but response format unexpected", "warning");
//       }
//     } catch (err) {
//       hideOverlay();
...
//       for (const qId of questionsToMove) {
//         try {
//           await moveQuestion(
//             qId,
//             "mcq_single_correct",
//             selectedTargetQB.qb_id,
//             selectedQB.qb_id
//           );
//           successCount++;
//           showOverlay(`📦 Moving: ${successCount}/${questionsToMove.length}`);
//           await sleep(300);
//         } catch (err) {
...
//       if (successCount > 0) {
//         showAlert(
//           `✅ Moved ${successCount} question(s) to ${selectedTargetQB.qb_name}${failCount > 0 ? ` (${failCount} failed)` : ""}`,
//           "success"
//         );
...
//       setSelectedQuestions(new Set()); // Clear selection when switching back to used view
//       setClonedQuestions([]); // Clear cloned questions when switching back
...
//       }
//       setShowAllQuestions(true);
//       setSelectedQuestions(new Set()); // Clear selection when switching to all view
//     }
//   };
...
//                             // Initially show only used questions
//                             setShowAllQuestions(false);
...
//                               // Store all questions
//                               setAllQuestions(allQuestions);
//                               // Filter to show only used questions initially
...
//               {/* Clone Button - Always show if we have selected questions */}
//               <button
...
//                 (Cloned questions are highlighted in green)
//               </span>
//             )}
//             {!showAllQuestions && (
//               <span className="qb-view-mode-hint">
...
//                         </span>
//                         <span>QB: {q.qb_id.slice(0, 8)}…</span>
//                       </div>
//                     </div>
//                   );
//                 })}
...
// // You'll need to create this config file or replace with your department IDs
// import { DEPARTMENT_IDS } from "../config";
// import "./CourseQBFinder.css";
// const API = "https://api.examly.io";
...
//   const [allQuestions, setAllQuestions] = useState([]);
//   const [usedQuestionIds, setUsedQuestionIds] = useState(new Set());
//   const [originalQuestionIds, setOriginalQuestionIds] = useState(new Map());
...
//     // Build a map of original question IDs
//     const originalIdMap = new Map();
//     questions.forEach(q => {
//       // Check if this is a cloned question (has imported field pointing to original)
//       if (q.imported && q.imported !== "original_question") {
//         // The imported field contains the original q_id
//         originalIdMap.set(q.q_id, q.imported);
//       } else {
//         // This is an original question
//         originalIdMap.set(q.q_id, q.q_id);
//       }
//     });
    
//     setOriginalQuestionIds(originalIdMap);
//     return questions;
//   }
...
//         try {
//           // Fetch all questions from QB (this will update originalQuestionIds map)
//           const allQuestions = await fetchQBQuestions(selectedQB.qb_id);
//           setAllQuestions(allQuestions);
//           setQbQuestions(allQuestions); // Show all questions
//           setSelectedQuestions(new Set(clonedQIds)); // Auto-select cloned questions
//           hideOverlay();
//           showAlert("✅ Now showing ALL questions from the QB. Cloned questions are highlighted in green.", "success");
//         } catch (err) {
//           hideOverlay();
//           showAlert("⚠️ Cloned but failed to fetch all questions: " + err.message, "warning");
//         }
...
//       {/* Alert */}
//       {alert && (
//         <div style={{...styles.alert, ...styles[`alert${alert.type.charAt(0).toUpperCase() + alert.type.slice(1)}`]}}>
//           {alert.msg}
//         </div>
//       )}
...
//       {/* Move Modal */}
//       {showMoveModal && (
//         <div style={styles.modalBackdrop} onClick={() => setShowMoveModal(false)}>
//           <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
//             <div style={styles.modalHeader}>
//               <h3 style={styles.modalTitle}>📦 Move Questions to QB</h3>
//               <button style={styles.modalClose} onClick={() => setShowMoveModal(false)}>
//                 ×
//               </button>
//             </div>
...
//                 <label style={styles.label}>Search Question Bank</label>
//                 <input
//                   type="text"
//                   value={moveSearchTerm}
//                   onChange={(e) => setMoveSearchTerm(e.target.value)}
...
//                       onClick={() => setSelectedTargetQB(qb)}
//                     >
//                       <div style={styles.searchItemName}>{qb.qb_name}</div>
...
//                   ...((!selectedTargetQB) && styles.buttonDisabled)
//                 }}
//               >
//                 Move {clonedQuestions.length > 0 ? clonedQuestions.length : selectedQuestions.size} Questions
//               </button>
...
//           <h2 style={styles.welcomeTitle}>🎓 Course Question Bank Finder</h2>
...
//           <button onClick={saveToken} style={styles.buttonPrimary}>
//             Save Token
//           </button>
...
//           <h3 style={styles.title}>🔍 Search Course</h3>
...
//           <button
//             onClick={clearToken}
//             style={{...styles.buttonDanger, marginTop: "12px"}}
//           >
//             🚪 Logout
//           </button>
...
//                 <h4 style={styles.subtitle}>📚 Modules & Tests</h4>
...
//                 {moduleTree.map((mod) => (
//                   <div key={mod.id} style={styles.moduleNode}>
//                     <div
...
//                       <span style={styles.testBadge}>{mod.totalTests} tests</span>
//                     </div>
...
//                             <div style={styles.sectionTitle}>📝 Direct Tests</div>
//                             </div>
//                         )}
...
//                   setAllQuestions([]);
//                   setUsedQuestionIds(new Set());
//                   setOriginalQuestionIds(new Map());
//                 }}
//                 className="qb-button qb-button-small qb-button-secondary"
//               >
//                 ← Back to QBs
...
//                   const isCloneOfSelected = clonedQuestions.some(clonedId => {
//                     const clonedOriginalId = originalQuestionIds.get(clonedId);
//                     const currentOriginalId = originalQuestionIds.get(q.q_id);
//                     return clonedOriginalId && currentOriginalId && clonedOriginalId === currentOriginalId && q.q_id !== clonedId;
//                   });
...
//                             </span>
//                           )}
//                           {isCloneOfSelected && !isCloned && (
//                             <span className="qb-original-badge" title="Original question that was cloned">
//                               📋 Original
//                             </span>
//                           )}
//                           {!showAllQuestions && isUsed && (
...
//                             </span>
//                           )}
//                           {showAllQuestions && isUsed && !isCloned && !isCloneOfSelected && (
//                             <span className="qb-used-badge" title="Used in selected tests">
//                               🎯
//                             </span>
//                           )}
