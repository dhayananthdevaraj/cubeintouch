...
import snippetJdbcTxt     from "../../public/templates/format_snippet_Jdbc_.txt?raw";
...
const FORMAT_TEMPLATES = [
  ...
  {
    id: "snippet-jdbc", icon: "🐬", label: "JDBC (MySQL)", sub: "Header · Footer · Whitelist · MySQL Schema",
    color: "#00758f", colorDim: "rgba(0,117,143,0.08)", colorBorder: "rgba(0,117,143,0.2)",
    filename: "format_snippet_Jdbc_.txt", content: snippetJdbcTxt,
  },
];
...
function ParseErrorsPanel({ errors, warnings, onDismiss }) {
  ...
  const accent      = hasErrors ? "#ef4444" : "#f59e0b";
  const accentSoft  = hasErrors ? "rgba(239,68,68,0.07)" : "rgba(245,158,11,0.07)";
  const accentGlow  = hasErrors ? "rgba(239,68,68,0.35)" : "rgba(245,158,11,0.35)";
  const showPipeTip = warnings.some(w => w.includes("pipe") || w.includes("|"));
  ...
  const Pill = ({ count, label, color, bg, border }) => (
    <span style={{
      ...
      fontSize: 11, fontWeight: 800, color, letterSpacing: "0.01em",
    }}>
    ...
    return (
      <div style={{
        ...
        <span style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          background: isErr ? "rgba(239,68,68,0.1)" : "rgba(245,158,11,0.1)",
          color: c, fontSize: 11, fontWeight: 900,
        }}>
          {isErr ? "✕" : "!"}
        </span>
        <span style={{
          color: "#334155", fontSize: 12.5, lineHeight: 1.55,
          fontFamily: "'Fira Code', 'Cascadia Code', monospace",
        }}>{text}</span>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 12px 34px rgba(15,23,42,0.07), 0 2px 6px rgba(15,23,42,0.04)",
    }}>
      {/* Header */}
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12,
        padding: "16px 18px",
        background: `linear-gradient(180deg, ${accentSoft}, rgba(255,255,255,0))`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            background: accent, color: "#fff", fontSize: 18, fontWeight: 900,
            boxShadow: `0 6px 16px ${accentGlow}`,
          }}>
            {hasErrors ? "✕" : "!"}
          </div>
          <div>
            <div style={{ fontSize: 14.5, fontWeight: 800, color: "#0f172a", letterSpacing: "-0.01em" }}>
              {hasErrors
            ...
            <div style={{ fontSize: 12, color: "#64748b", fontWeight: 500, marginTop: 2 }}>
              {hasErrors
                ? "Resolve the errors below, then parse again."
                : "Parsing succeeded — review the notes below."}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {hasErrors && (
            <Pill count={errors.length} label={errors.length > 1 ? "errors" : "error"}
                  color="#dc2626" bg="rgba(239,68,68,0.08)" border="rgba(239,68,68,0.22)" />
          )}
          {warnings.length > 0 && (
            <Pill count={warnings.length} label={warnings.length > 1 ? "warnings" : "warning"}
                  color="#d97706" bg="rgba(245,158,11,0.09)" border="rgba(245,158,11,0.25)" />
          )}
          <button
            ...
            style={{
              ...
              background: "#f3f4f7", border: "1px solid #e7e9ee", color: "#64748b",
              fontSize: 17, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center",
              transition: "all 0.15s", flexShrink: 0,
            }}
            onMouseEnter={e => { e.currentTarget.style.background = "#e7e9ee"; e.currentTarget.style.color = "#334155"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "#f3f4f7"; e.currentTarget.style.color = "#64748b"; }}
          >×</button>
        </div>
      </div>
      ...
      <div style={{ padding: "14px 16px 16px" }}>
        {hasErrors && (
          <div style={{ marginBottom: warnings.length > 0 ? 16 : 0 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {errors.map((err, i) => <Row key={i} kind="error" text={err} />)}
            </div>
          </div>
        )}
        ...
        {warnings.length > 0 && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {warnings.map((w, i) => <Row key={i} kind="warning" text={w} />)}
            </div>
          </div>
        )}
        ...
        {showPipeTip && (
          <div style={{
            ...
            <span>
              <strong>Tip:</strong> If your expected output contains <code>|</code> characters, use{" "}
              <code style={{ background: "#e0e7ff", padding: "1px 5px", borderRadius: 5, fontFamily: "monospace" }}> | </code>{" "}
              (space · pipe · space) as the field delimiter — a bare <code>|</code> inside output is safe as long as the delimiters have spaces.
            </span>
          </div>
        )}
      </div>
...
const ALLOWED_LANGUAGES = [
  "Bash", "C#", "C", "C++", "Clojure", "Dotnet", "Go", "Java", "Java_jdbc",
  "MySQL", "Objective-C", "Perl", "PHP", "Plain Javascript", "Python", "Ruby",
  "Rust", "VB.NET", "R", "MySQL Verbose", "Cc++11", "C++c++11", "Java17",
  "Java21", "Kotlin",
];
...
function parseQuestions(raw) {
  ...
  const HEADER_RE = /^(TITLE|DIFFICULTY|LANGUAGE|BLOOMS|TAGS|DESCRIPTION|INPUT_FORMAT|OUTPUT_FORMAT|CONSTRAINTS|CODE_STUB\[[^\]]+\]|SOLUTION\[[^\]]+\]|HEADER\[[^\]]+\]|FOOTER\[[^\]]+\]|WHITELIST\[[^\]]+\]|BLACKLIST\[[^\]]+\]|MYSQL_SCHEMA\[[^\]]+\]|TESTCASE|SAMPLE_IO)\s*:/i;
  ...
  blocks.forEach((block, bi) => {
    ...
    // MYSQL_SCHEMA[Java_jdbc]: — DDL/seed data the JDBC solution connects to
    // (ri_db by default); sent through as `mysqlSchema` on that language's
    // solution entry, same shape Examly's own JDBC-authored questions use.
    const mysqlSchemas = getLangMap("MYSQL_SCHEMA");
    ...
    declaredLangs.forEach(l => {
      if (!ALLOWED_LANGUAGES.includes(l)) {
        const ci = ALLOWED_LANGUAGES.find(a => a.toLowerCase() === l.toLowerCase());
        qErrors.push(
          `Q${qNum}: LANGUAGE "${l}" is not a supported identifier (case-sensitive).` +
          (ci ? ` Did you mean "${ci}"?` : "")
        );
      }
    });

    // ── SOLUTION language must exactly match a declared LANGUAGE (case-sensitive) ──
    Object.keys(solutions).forEach(solLang => {
      if (!declaredLangs.includes(solLang)) {
        const ci = declaredLangs.find(a => a.toLowerCase() === solLang.toLowerCase());
        qErrors.push(
          `Q${qNum}: SOLUTION[${solLang}] doesn't match any declared LANGUAGE (case-sensitive).` +
          (ci ? ` LANGUAGE has "${ci}" — SOLUTION must use the exact same word.` : "")
        );
      }
    });

    // ── HEADER/FOOTER/CODE_STUB/WHITELIST/BLACKLIST keys should match a declared language ──
    [["HEADER", headers], ["FOOTER", footers], ["CODE_STUB", codeStubs], ["WHITELIST", whitelists], ["BLACKLIST", blacklists], ["MYSQL_SCHEMA", mysqlSchemas]].forEach(([label, map]) => {
      Object.keys(map || {}).forEach(k => {
        if (!declaredLangs.includes(k))
          qWarns.push(`Q${qNum}: ${label}[${k}] language doesn't match a declared LANGUAGE — it will be ignored.`);
      });
    });
...
function buildPayload(q, batchConfig, qbId, userId) {
  ...
  // Each line under WHITELIST[Lang]/BLACKLIST[Lang] becomes ONE combined list —
  // every comma-separated term on that line goes into the SAME list array.
  // A second line (if present) becomes a SECOND, separate list entry.
  //   WHITELIST[Java21]:
  //   try,catch,ArithmeticException      -> { list: ["try","catch","ArithmeticException"] }
  //   sample1,sample2,sample3            -> { list: ["sample1","sample2","sample3"] }   (2nd entry)
  const parseList = raw => {
    if (!raw?.trim()) return [];
    return raw
      .split("\n")
      .map(line => line.trim())
      .filter(Boolean)
      .map(line => ({ list: line.split(",").map(v => v.trim()).filter(Boolean) }))
  ...
  const solutionArray = q.languages.map(lang => {
    ...
    return {
      ...
      ...(q.mysqlSchemas?.[lang] ? { mysqlSchema: q.mysqlSchemas[lang] } : {}),
              ...
              <div style={{ background:"#fffbeb", border:"1.5px solid #fde68a", borderRadius:14, padding:"14px 16px", marginBottom:16 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                  ...
                  <span style={{ fontSize:12.5, fontWeight:800, color:"#92400e" }}>
                    Supported languages — use these exact words (case-sensitive)
                  </span>
                </div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:10 }}>
                  {ALLOWED_LANGUAGES.map(l => (
                    <span key={l} style={{ fontFamily:"'Fira Code', 'Cascadia Code', monospace", fontSize:11, fontWeight:700, color:"#7c2d12", background:"#fff7ed", border:"1px solid #fed7aa", borderRadius:6, padding:"3px 8px" }}>{l}</span>
                  ))}
                </div>
                <div style={{ fontSize:11, color:"#78350f", lineHeight:1.6 }}>
                  ...
                  <br />
                  <strong>MYSQL_SCHEMA[Java_jdbc]:</strong> — required for JDBC questions. The table-creation + seed-data DDL the solution's <code>DriverManager.getConnection(...)</code> connects to (default workspace DB: <code>jdbc:mysql://localhost/ri_db</code>). Sent as <code>mysqlSchema</code> on that language's solution entry:
                  <br />
                  <code style={{ display: "block", marginTop: 4, background: "#fff7ed", padding: "6px 8px", borderRadius: 4, whiteSpace: "pre" }}>{"MYSQL_SCHEMA[Java_jdbc]:\nCREATE TABLE shipment (\n    id INT AUTO_INCREMENT PRIMARY KEY,\n    item_name VARCHAR(255) NOT NULL\n);\n\nINSERT INTO shipment (item_name) VALUES ('Laptops');"}</code>
                </div>
              </div>
