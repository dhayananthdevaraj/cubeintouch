...
export default function QBAccess() {
  ...
  async function fetchAllQBQuestionsWithRetry(qbId, expectedCount = null, maxRetries = 8) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      // If we have expected count, check if we got all questions
      if (expectedCount) {
        ...
        // No expected count, just check if we got any questions
        if (questions.length > 0) {
          console.log("✅ Questions successfully fetched!");
          return questions;
        }
      ...
      if (attempt < maxRetries) {
        // Exponential backoff: 2s, 4s, 6s, 8s, 10s, 12s, 14s
        const waitTime = attempt * 2000;
        console.log(`⏳ Waiting ${waitTime/1000}s before retry...`);
        await sleep(waitTime);
      }
  ...
  const handleCloneQB = async () => {
    ...
    try {
      ...
      // Use retry mechanism to wait for ALL questions to appear
      // This will automatically retry up to 8 times (2s, 4s, 6s, 8s, 10s, 12s, 14s, 16s)
...
const handleMoveAndDelete = async () => {
    ...
    try {
      // Step 1: Move all questions in bulk batches
      showOverlay(`📦 Moving ${questionCount} question(s)...`);
      ...
      // Process in batches of 50 questions per API call (API supports multiple IDs)
      const batchSize = 50;
      let successCount = 0;
      let failCount = 0;
      ...
      for (let i = 0; i < questionIds.length; i += batchSize) {
        ...
        try {
          // Move entire batch in one API call
          ...
          // If batch fails, try moving questions one by one as fallback
          ...
          for (let j = 0; j < batchIds.length; j++) {
            try {
              ...
              successCount++;
              failCount--; // Reduce fail count since we succeeded
            } catch (individualErr) {
              console.error(`Failed to move question ${batchIds[j]}:`, individualErr);
            }
            await sleep(100); // Small delay between individual retries
          }
        }
