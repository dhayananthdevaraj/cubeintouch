...
export default function MCQUploaderPlatform({ platform, onBack }) {
  ...
  const saveToken = () => {
    ...
    try {
      localStorage.setItem(platform.tokenKey, tokenInput.trim());
  ...
  // Filter departments based on search — sourced from the active platform
  const filteredDepartments = (platform.bdIdOptions || []).filter(dept =>
      ...
      {ui === "welcome" && (
        <div className="mcq-welcome">
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            ...
            <button onClick={onBack} className="mcq-button mcq-button-secondary">
              ← Back
            </button>
          </div>

          <p className="mcq-token-hint">
            💡 Token saved separately per platform
          </p>
        </div>
      )}
      ...
      {ui === "results" && uploadResults && (
        <div className="mcq-card">
          <div className="mcq-result-section">
            <div style={{ marginBottom: 6 }}><PlatformBadge /></div>
