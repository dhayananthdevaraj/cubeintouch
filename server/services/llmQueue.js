...
//   const apiKey = provider.apiKey; // ← uses per-provider key (supports gemini-1 and gemini-2)
...
// ← FIXED: added 30s timeout to prevent Premature close from hanging forever
async function callGroq(provider, { model, messages, temperature, max_tokens }) {
...
export async function llmCall({
  ...
  for (const provider of PROVIDERS) {
    ...
    try {
      ...
      // ← FIXED: catch network drops and fail fast instead of hanging
      const isNetworkDrop =
        err.name === "AbortError"                  ||
      ...
      if (isNetworkDrop) {
        console.warn(`  ⚠️  [${provider.name}] Network drop / timeout. Skipping to next provider.`);
