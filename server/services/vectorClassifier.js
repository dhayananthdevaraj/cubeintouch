...
export async function classifyQuestion(questionText, token) {
  ...
  try {
    ...
    // ✅ FIX 1: GUARD — if any collection returned empty results, fail gracefully
    if (!subjectResults.length || !topicResults.length || !subtopicResults.length) {
      console.error("❌ One or more Qdrant collections returned empty results.");
      console.error(`   Subjects: ${subjectResults.length}, Topics: ${topicResults.length}, Subtopics: ${subtopicResults.length}`);
      throw new Error(
        "Vector collections appear empty or not yet indexed. Please wait for indexing to complete and retry."
      );
    }
    ...
    // ✅ Get top matches (safe — already guarded above)
    const topSubject = subjectResults[0];
    const topTopic = topicResults[0];
    const topSubtopic = subtopicResults[0];
    ...
    // ✅ FIX 2: Stringify IDs for consistent Qdrant filter matching
    const topTopicId = String(topTopic.payload.topic_id);
    const topSubjectId = String(topSubject.payload.subject_id);

    // ✅ SMART SELECTION STRATEGY
    let finalResult;
    let strategy;

    if (subtopicScore >= 60 && String(topSubtopic.payload.topic_id) === topTopicId) {
      // Direct match — subtopic and topic are aligned
      finalResult = topSubtopic.payload;
      strategy = "direct_match";
      
    } else if (topicScore >= 50) {
      // Filter subtopics under the best topic
      let filteredSubtopics = [];
      try {
        filteredSubtopics = await qdrant.search(collections.subtopics, {
          vector: questionVector,
          limit: 1,
          filter: {
            must: [{ key: "topic_id", match: { value: topTopicId } }]
          },
          with_payload: true
        });
      } catch (filterErr) {
        console.warn(`   ⚠️ topic_filtered search failed: ${filterErr.message}`);
      }

      // ✅ FIX 3: Safe fallback if filtered result is empty
      finalResult = (filteredSubtopics.length > 0)
        ? filteredSubtopics[0].payload
        : topSubtopic.payload;
      strategy = "topic_filtered";
      
    } else if (subjectScore >= 40) {
      // Chain: subject → topic → subtopic
      let filteredTopics = [];
      try {
        filteredTopics = await qdrant.search(collections.topics, {
          vector: questionVector,
          limit: 1,
          filter: {
            must: [{ key: "subject_id", match: { value: topSubjectId } }]
          },
          with_payload: true
        });
      } catch (filterErr) {
        console.warn(`   ⚠️ subject_chain topic search failed: ${filterErr.message}`);
      }

      if (filteredTopics.length > 0) {
        // ✅ FIX 4: Stringify the chained topic_id too
        const chainedTopicId = String(filteredTopics[0].payload.topic_id);
        let filteredSubtopics = [];
        try {
          filteredSubtopics = await qdrant.search(collections.subtopics, {
            vector: questionVector,
            limit: 1,
            filter: {
              must: [{ key: "topic_id", match: { value: chainedTopicId } }]
            },
            with_payload: true
          });
        } catch (filterErr) {
          console.warn(`   ⚠️ subject_chain subtopic search failed: ${filterErr.message}`);
        }
        ...
        // No matching topic found under subject — use best overall subtopic
        finalResult = topSubtopic.payload;
        strategy = "fallback";
      }
    ...
    // ✅ FIX 5: Guard finalResult in case all paths somehow produced undefined
    if (!finalResult) {
      console.warn("   ⚠️ finalResult was undefined — falling back to topSubtopic");
      finalResult = topSubtopic.payload;
