...
function runScriptInFolder(folderPath, scriptContent, stack) {
  ...
  // Write script into folder.
  // NOTE: we intentionally do NOT pass { mode: 0o755 } here — fs.writeFileSync's
  // mode option is a Windows/Node-side instruction and is not reliably honored
  // once the file is accessed from WSL across the DrvFS boundary. We set the
  // executable bit explicitly via the same shell() helper that later runs the
  // script, so permissions and execution both happen in the same context.
  const scriptPath = path.join(folderPath, "script.sh");
  fs.writeFileSync(scriptPath, scriptContent);
  shell(`chmod +rx '${folderSh}/script.sh'`);
...
export async function runPackager(stack, buffer, originalName) {
  ...
  try {
    ...
    // FIX: some zips carry restrictive Unix permissions on their entries
    // (e.g. read-only folders), which unzip faithfully restores. That can
    // cause Node's later fs.writeFileSync (writing script.sh into a nested
    // folder) to fail with EACCES before the script even runs. Normalize
    // everything we just extracted to be writable for the owner.
    // u+rwX -> owner gets read/write; execute bit only applied to dirs/files
    // that already have some execute bit set (safer than blanket +rwx).
    shell(`chmod -R u+rwX '${extractDirSh}'`);
