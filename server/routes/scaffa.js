...
import {
  ...
  generateReactZip,
  generateAngularZip,
  generateSpringBootZip,
  generateAngularSpringBootZip,
  downloadFolderAsZip,
...
// ── POST /scaffa/upload-springboot-scaf  →  SpringBoot ZIP (springapp + junit) ─
router.post("/upload-springboot-scaf", upload.single("testCase"), async (req, res) => {
  const testCase    = req.file ? req.file.buffer.toString() : req.body.testCase;
  const zipFileName = req.body.zipFileName;
  ...
  try {
    const { zipPath, tempDir } = await generateSpringBootZip(testCase, zipFileName);
    res.download(zipPath, `${zipFileName}.zip`, async (err) => {
      if (err) console.error("❌ SpringBoot ZIP download error:", err.message);
      await fs.remove(tempDir).catch(() => {});
      console.log(`🧹 Cleaned up: ${tempDir}`);
    });
  } catch (err) {
    console.error("❌ /upload-springboot-scaf:", err.message);
    res.status(500).json({ error: "Failed to generate SpringBoot ZIP" });
  }
});

// ── POST /scaffa/upload-angular-springboot-scaf → combined ZIP (all 4 folders) ─
router.post(
  "/upload-angular-springboot-scaf",
  upload.fields([{ name: "specFiles" }, { name: "testCase", maxCount: 1 }]),
  async (req, res) => {
    const specFiles   = req.files?.specFiles || [];
    const testCaseFile = req.files?.testCase?.[0];
    const javaTestCase = testCaseFile ? testCaseFile.buffer.toString() : req.body.testCase;
    const zipFileName  = req.body.zipFileName;

    if (!zipFileName)        return res.status(400).json({ error: "zipFileName is required" });
    if (!specFiles.length)   return res.status(400).json({ error: "No Angular spec files uploaded" });
    if (!javaTestCase)       return res.status(400).json({ error: "testCase (SpringBoot test file) is required" });

    try {
      const { zipPath, tempDir } = await generateAngularSpringBootZip(specFiles, javaTestCase, zipFileName);
      res.download(zipPath, `${zipFileName}.zip`, async (err) => {
        if (err) console.error("❌ Angular+SpringBoot ZIP download error:", err.message);
        await fs.remove(tempDir).catch(() => {});
        console.log(`🧹 Cleaned up: ${tempDir}`);
      });
    } catch (err) {
      console.error("❌ /upload-angular-springboot-scaf:", err.message);
      res.status(500).json({ error: "Failed to generate Angular+SpringBoot ZIP" });
    }
  }
);
